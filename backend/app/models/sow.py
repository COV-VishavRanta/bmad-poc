...
class SOW(BaseModel):
    ...
    status = Column(
        Enum(SOWStatus), nullable=False, default=SOWStatus.DRAFT, index=True
    )
    ...
    roles = relationship("SOWRole", back_populates="sow", cascade="all, delete-orphan")

    projects = relationship("Project", back_populates="sow")
    groups = relationship("Group", back_populates="sow")

    history = relationship(
        "SOWHistory", back_populates="sow", cascade="all, delete-orphan"
    )
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "start_date <= end_date OR start_date IS NULL OR end_date IS NULL",
...
class SOWRole(BaseModel):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "hourly_rate IS NULL OR hourly_rate > 0", name="chk_sow_role_rate_positive"
        ),
    )
