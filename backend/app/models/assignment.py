...
class Assignment(BaseModel):
    ...
    @classmethod
    def get_user_fte_total(
        cls, db_session, user_id: int, start_date: date, end_date: date
    ) -> Decimal:
        """
        ...
        total_fte = sum(
            assignment.fte_allocation for assignment in overlapping_assignments
        )
    ...
    def validate_fte_constraint(self, db_session) -> bool:
        ...
        total_fte = self.get_user_fte_total(
            db_session, self.user_id, self.start_date, self.end_date
        )
